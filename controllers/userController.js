...
const addMealToPlan = async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({ message: 'One or more meal IDs are invalid.' });
        }
        ...
        await user.save();
        res.json({ message: 'Meal plan updated successfully', mealPlan: user.mealPlan });
...
const clearMealPlan = async (req, res) => {
    try {
        ...
        await user.save();
        res.json({ 
            message: day ? `Meal plan for ${day} cleared successfully` : "Meal plan cleared successfully",
